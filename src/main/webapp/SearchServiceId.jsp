...
int totalNumberofRecord = ClientOperation.totalNumberOfSameServiceName(request.getParameter("client_identity"));
int totalPage = 0;
...
List<Client> clientList = ClientOperation.search(request.getParameter("client_identity"),startFrom, total);
%>
    ...
    <center><h4 style='color:teal'>ServiceID List [Total page <%=pageno%>/<%=totalPage%>]</h4></center>
        ...
        if(totalPage > 1) {
            if(pageNo >= 1 && pageNo <= totalPage) {%>
                <center><h4>
                <%if (pageNo >1 && pageNo < totalPage) {%>
                    <a href='SearchServiceId.jsp?currentPage=<%=1%>&searchPage=<%=request.getParameter("searchPage") %>&client_identity=<%=request.getParameter("client_identity") %>'><button class='previous'>  &laquo; First page</button></a>
                    <a href='SearchServiceId.jsp?currentPage=<%= --pageNo %>&searchPage=<%=request.getParameter("searchPage") %>&client_identity=<%=request.getParameter("client_identity") %>'><button class='previous'>  &laquo; Previous Page</button></a>
                    <%int nextpageNo = pageNo+2;%>
                    <a href='SearchServiceId.jsp?currentPage=<%= nextpageNo %>&searchPage=<%=request.getParameter("searchPage") %>&client_identity=<%=request.getParameter("client_identity") %>'><button class='next'>Next Page &raquo;</button></a>
                    <a href='SearchServiceId.jsp?currentPage=<%= totalPage %>&searchPage=<%=request.getParameter("searchPage") %>&client_identity=<%=request.getParameter("client_identity") %>'><button class='next'>Last Page &raquo;</button></a>
                <%}	else if(pageNo == 1) {%>
                    <button class='disable' disabled>First page</button>
                    <button class='disable' disabled>Previous Page</button>
                    <a href='SearchServiceId.jsp?currentPage=<%= ++pageNo %>&searchPage=<%=request.getParameter("searchPage") %>&client_identity=<%=request.getParameter("client_identity") %>'><button class='next'>Next Page &raquo;</button></a>
                    <a href='SearchServiceId.jsp?currentPage=<%= totalPage %>&searchPage=<%=request.getParameter("searchPage") %>&client_identity=<%=request.getParameter("client_identity") %>'><button class='next'>Last Page &raquo;</button></a>
            ...
            <%} else if (pageNo > totalPage) {%>
                ...
                <a href='SearchServiceId.jsp?currentPage=<%= totalPage %>&searchPage=<%=request.getParameter("searchPage") %>&client_identity=<%=request.getParameter("client_identity") %>'><button class='previous'> &laquo; Last Page</button></a>
                </center>
            <%} else if (pageNo < 1){%>
                <center><h4 >No data found. Please go to first page.</h4>
                <a href='SearchServiceId.jsp?currentPage=<%=1 %>&searchPage=<%=request.getParameter("searchPage") %>&client_identity=<%=request.getParameter("client_identity") %>'><button class='next'> First Page&raquo;</button></a></center>
            <%}
        }%>
    <center>
    <div style='padding-bottom:1%'>
        <form name='SearchForm' method="get" id='search' onsubmit="return validateSearchForm()">
            <table align="center" style='width:10%'>
                ...
                <td align="left" ><input  type='text' placeholder="Enter service id or service name or customer name" name='client_identity' value='<%=request.getParameter("client_identity")%>' autofocus/></td>
    ...
    <div style='overflow-x:auto'>
        ...
        if (clientList.isEmpty()) {
            if (request.getParameter("searchPage").equals("1")) {
                request.setAttribute("IncorrectClientIdentity",request.getParameter("client_identity"));
                request.setAttribute("DataNotFound", "This service id or service name does not exist in database.");
                request.getRequestDispatcher("ClientListByMonth.jsp?currentPage=1&from=start&to="+new SimpleDateFormat("yyyy-MM-dd").format(new Date())).forward(request,response);
            } else if (request.getParameter("searchPage").equals("2")) {
                request.setAttribute("IncorrectClientIdentity",request.getParameter("client_identity"));
                request.setAttribute("DataNotFound", "Sorry for inconvenience. This service id or service name does not exist in database.");
