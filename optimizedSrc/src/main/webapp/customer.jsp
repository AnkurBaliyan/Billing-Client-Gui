...
if (phone != null && "delete".equals(action)) {
	CustomerOperation.deleteCustomer(phone);
}
List<Customer> list = CustomerOperation.getCustomer(null);
%>
    ...
    <%} else {%>
        <div style="padding-right:30px;padding-top:.5%;padding-bottom:1%">
            <form name="SearchForm" method="get" id="search" onsubmit="return validateSearchForm()">
                <table align="center" style="width:100%">
                    ...
                    <td align="left" ><input  type='text' placeholder="Enter site id or site name or user mobile" name='site_identity'  <% if(null != request.getParameter("site_identity")){%>value="<%=request.getParameter("site_identity") %>"<%} %> pattern="^[a-zA-Z1-9].*" title="Remove special character from starting." style="height:35px;width:99%;" autofocus /></td>
        ...
        if (request.getParameter("site_identity") !=  null){
            list = CustomerOperation.getCustomer(request.getParameter("site_identity"));
            if (list.isEmpty()) {%>
        ...
        <div style='max-width:1100px;'>
            <table border="1" style="width:100%;border:3px solid #0795d1">
                <tr><th >Site Id</th><th >Site Name</th><th >User Name</th><th >Email Id</th><th>Phone</th><th>Update</th><th>Delete</th>
                <%
                for (Customer customer : list) {%>
                    <tr>
                    <td><%=customer.getSiteId() %></td><td><%=customer.getSiteName()%></td><td><%=customer.getUserName()%></td>
                    <td><%=customer.getEmailId()%></td><td><%=customer.getPhone()%></td>
                    <td><a href="updateCustomer.jsp?phone=<%=customer.getPhone()%>" style="text-decoration: none; color: blue">Update</a></td>
                    <td><a href="customer.jsp?phone=<%=customer.getPhone()%>&action=delete" onclick="return confirm('Do you want to delete this customer?');" style="text-decoration: none; color: #FC0000">Delete</a></td>
                    </tr>
                <%}%>
            </table>
        </div>
